export async function onRequest(context) {
  ...
  try {
    ...
    if (path.startsWith('/api/clients')) {
      return handleClients(request, env, path, corsHeaders);
    }

    if (path.startsWith('/api/quotations')) {
      return handleQuotations(request, env, path, corsHeaders);
    }

    if (path.startsWith('/api/materials')) {
      return handleMaterials(request, env, path, corsHeaders);
    }

    if (path.startsWith('/api/sync')) {
      return handleSync(request, env, path, corsHeaders);
    }
...
async function handleClients(request, env, path, corsHeaders) {
  const db = env.DB;
...
async function handleQuotations(request, env, path, corsHeaders) {
  const db = env.DB;
  const tenantId = 'default-tenant';
...
async function handleMaterials(request, env, path, corsHeaders) {
  const db = env.DB;
  const tenantId = 'default-tenant';
